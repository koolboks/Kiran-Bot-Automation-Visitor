...
async def load_manual_setting():

    with open('settings.json', 'r') as fp:
       data = (json.load(fp)).get('MANUAL',False )
       return data
...
async def third_page(update, page, data):
    ...
    async def handle_other_names():
        ...
        # You can add your code here
        is_other_names = data.get("OtherNames", "")  # Assuming "OtherNames" is the key for the yes/no value
        log.info("OTher names.....", is_other_names)
        if is_other_names.lower() == "yes":
            ...
            await delay()
            # JavaScript code to click the button using getElementById
            js_code = """
            document.getElementById('315e92cb-b970-eb11-a812-000d3acba81e_TRUE').click();
            """
            ...
            # # Click on the radio button for "No"
            # await page.click('input[id="315e92cb-b970-eb11-a812-000d3acba81e_FALSE"]')
            js_code = "document.getElementById('315e92cb-b970-eb11-a812-000d3acba81e_FALSE').click();"
            await page.evaluate(js_code)
            # Add your code here for actions to be performed if "No" is selected
    ...
    async def Passport_and_birth_details():
        ...
        # Have you ever spent time in the Issue country?
        is_country_lived = data.get("HasLiveInCountry", 'yes')
        if is_country_lived.lower() == 'no':
            js_code = "document.getElementById('5e2c37fc-07e2-ec11-bb3d-002248112af0_FALSE').click();"
            await page.evaluate(js_code)
        else:
            js_code = "document.getElementById('5e2c37fc-07e2-ec11-bb3d-002248112af0_TRUE').click();"
            await page.evaluate(js_code)
        ...
        if issue_date:
            # Split the date into day, month, and year
            day, month, year = re.split(r"[-/]", issue_date) #issue_date.split("-")
            # Fill day input field
            await page.type('input[id="ca0f4ece-c070-eb11-a812-000d3acba96b_date_dd"]', day)
            # Fill month input field
            await page.type('input[id="ca0f4ece-c070-eb11-a812-000d3acba96b_date_mm"]', month)
        ...
        if expiry_date:
            # Split the date into day, month, and year
            day, month, year = re.split(r"[-/]", expiry_date) # expiry_date.split("-")
            # Fill day input field
            await page.type('input[id="518553e0-c070-eb11-a812-000d3acba96b_date_dd"]', day)
            # Fill month input field
            await page.type('input[id="518553e0-c070-eb11-a812-000d3acba96b_date_mm"]', month)
        ...
        if dob:
            # Split date into day, month, and year
            day, month, year = re.split(r"[/-]", dob)#dob.split('-')
            ...
            if country_of_birth:
                ...
                if state_province_region_of_birth:
                    ...
                    await page.focus('input[name="32d8a69d-be70-eb11-a812-000d3acba96b"]')
                    await page.keyboard.type(state_province_region_of_birth)
                    await press_enter(page)
                else:
                    print("State/Province/Region of birth is empty in the data.")
    ...
    async def handle_new_contact_details_country():
        ...
        if current_country:
            ...
            state_or_province = data.get("CurrentProvince", "")
            try:
                await update.edit_text(f"Filling address: {state_or_province}")
                await page.type('input[name="823ce22c-5e88-eb11-a812-000d3acbe28e_GRP01045_000"]', state_or_province)
                # await press_enter(page)  # press enter
            except:
                pass
    ...
    async def handle_national_identity():
        ...
        if has_national_id == "yes":
            ...
            country_of_issue = data.get('CountryOfIssue', '')
            # Input the country or territory of issue into the input field
            await page.type('input[name="9da8b14a-c170-eb11-a812-000d3acba96b"]', country_of_issue)
            await delay()
            await press_enter(page)
...
async def main(update=None, bot=None):
    try:
        async with async_playwright() as p:
            ...
            await page.goto("https://apply.immigration.govt.nz/visa_dashboard/")
            # await page.goto('https://apply.immigration.govt.nz/mcs-wizard/?id=e864c092-a16a-40d4-ac11-82fcfd379493')
